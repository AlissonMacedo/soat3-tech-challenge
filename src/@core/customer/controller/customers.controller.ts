...
import { UpdateCustomerDto } from '../entities/update-customer.dto';
import { CreateCustomerDto } from '../entities/create-customer.dto';
...
export class CustomersController implements ICustomersController {
  ...
  async create(createCustomertDto: CreateCustomerDto) {
    const product = await this.customersService.findByCpf(
